...
class SerializationTests: XCTestCase {
    ...
    func test_01_encode() throws {
        ...
        case let .success(data):
            ...
            XCTAssert(str! == "{\"value\":\"value\"}", "Validation on Encoded Value Failed")
    ...
    func test_03_decode() throws {
        ...
        switch result {
        case let .success(myclass):
            logger.info("Decoded: \(myclass.description)")
            XCTAssert(myclass.description == "MyClass[value: value]", "Validation on Decoded Value Failed")
        case let .failure(error):
